...
func newClient() (observerpb.ObserverClient, func() error, error) {
	var gRPC string
	if val, ok := os.LookupEnv(hubbleServiceName); ok {
		gRPC = val
	} else {
		return nil, nil, fmt.Errorf("hubble service name variable must %q be set", hubbleServiceName)
	}
...
func getFlows(ctx context.Context, client observerpb.ObserverClient, reportChan chan *api.Item, req *observerpb.GetFlowsRequest) error {
